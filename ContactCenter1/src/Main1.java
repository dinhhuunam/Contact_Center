...
//        }
    ...
    //sắp xếp mảng agent theo thứ tự tăng dần của thời gian hangupat
    public static void sortListAgent(List<Agent> list) {
        list.sort((o1, o2) -> {
            // Xử lý trường hợp giá trị null
            if (o1.hangup == null && o2.hangup == null) {
                return 0;
            } else if (o1.hangup == null) {
                return -1; // Giá trị null luôn nhỏ hơn giá trị không null
            } else if (o2.hangup == null) {
                return 1; // Giá trị không null luôn lớn hơn giá trị null
            } else {
                // So sánh hai giá trị LocalTime
                return o1.hangup.compareTo(o2.hangup);
            }
        });
    }
                    ...
                    if (checkTime(list_csv, current)){
                        ...
                        for(int h=1; h<=cnt; h++){
                            ...
                            if (!checkStatus(list_pickUp, current, res)) {
                                ...
                                System.out.println(call);
                                list_kq.add(call);
                                //Call old_call = findCall(list_csv,current);
                                //System.out.println(old_call);
                                //System.out.println(list_csv.size());
                                //list_csv.remove(old_call);
                                //System.out.println(list_csv.size());
...
//                                list_csv.remove(old_call);
                                ...
                                //hiện tại không có agent nào rảnh thì lưu vào list các cuộc gọi đang chờ
                                    ...
                                    list_wait.add(call);
                                    //saveListCall(list_csv,list_wait,current,getStartTime(list_csv,current));
                                    System.out.println("List Wait: ");
//                                    for(Call call1 : list_wait){
...
//                            System.out.println("Số lượng cuộc list_csv: "+list_csv.size());
//                            System.out.println("Agent ranh: "+checkAgent(list_agent));
//                            System.out.println("so cuoc quay: "+(int) (countAgent(list_agent) * spin_call));
                            Agent agent1 = checkCallAgent(list_agent,current);
                            //đến thời điểm này tất cả các agent đều rảnh
                            //updateAgent(list_agent,current);
        ...
        try (FileWriter writer = new FileWriter("E:\\ContactCenter1\\data.csv")) {
            // Ghi header
            writer.append("agent_id,customer_id,call_at,pickup_at,transfer_at,hangup_at,status");
            writer.append("\n");
