...
public class Main {
    ...
    public static void addCallCsv(List<Call> list_call, LocalTime start, int num) {
        ...
        while (calls.size() < num ) {
            ...
            Call call = new Call(null,0,start,time,null,null,null,null);
            calls.add(call);
        }
    ...
    public static void saveListCall(List<Call> list,List<Call> list_wait,LocalTime time, LocalTime start){
        for(Call call : list){
            if(time.equals(call.pick_up_at)){
                list_wait.add(new Call(null,0,start,time,null,null,null,null));
            }
        }
    }
    //hàm để tính tỷ lệ pick_up
    public static double pickUp(List<Call> list_spin){
        int so_cuoc_connect = 0;
        for(Call call : list_spin){
            if(call.status.equals("connected")||call.status.equals("abandon")) so_cuoc_connect++;
        }
        double pick_up =  ((double) so_cuoc_connect/500);
        return pick_up;
    }
...
//        }
        ...
        //List lưu lại để tiến hành spin_call
        List<Call> list_spin = new ArrayList<>();
        int so_cuoc_quay = 0;
        int dem_call = 0;
        double pick_up_rate = 0.5;
        for (int hour = 8; hour <= 20; hour++) {
            for (int minute = 0; minute <= 59; minute++) {
                for (int second = 0; second <= 59; second++) {
                    ...
                    //tiến hành lấy spin_call theo từng khung thời gian
                    if(list_spin.size() >= 500){
                        double pick_up = pickUp(list_spin);
                        if(pick_up >=0 && pick_up <=0.2) pick_up = 0.2;
                        else if(pick_up>0.2 && pick_up <=0.3) pick_up = 0.3;
                        else if(pick_up >0.3 && pick_up <=0.4) pick_up = 0.4;
                        else if(pick_up > 0.4 && pick_up <=0.5) pick_up = 0.5;
                        else if(pick_up > 0.5 && pick_up <= 0.7) pick_up = 0.7;
                        else pick_up = 0.8;
                        pick_up_rate = pick_up;
                        spin_call = (double)1/pick_up;
                        list_spin.clear();
                        System.out.println("Picup-Spincall: "+pick_up+" "+spin_call);
                    }
                    ...
                    if (checkTime(list_csv, current)){
                        ...
                        for(int h=1; h<=cnt; h++){
                            ...
                            if (!checkStatus(list_pickUp, current, res)) {
                                Call call = new Call(null,0, getStartTime(list_csv,current), null, null,addSecondsToTime(getStartTime(list_csv,current)), "unconnected",Double.toString(spin_call));
                                //list_kq.add(call);
                                //if(call.call_at.isBefore(LocalTime.of(21,0,3))){
                                    list_kq.add(call);
                                    list_spin.add(call);
                                //}
                            }
                            //nếu khách hàng có nghe máy
                                ...
                                if (checkAgent(list_agent) != null) {
                                    ...
                                    Call call = new Call(Integer.toString(agent1.id),0, getStartTime(list_csv,current), current, current, hangup_at, "connected",Double.toString(spin_call));
                                    //list_kq.add(call);
                                    //if(call.call_at.isBefore(LocalTime.of(21,0,3))){
                                        list_kq.add(call);
                                        list_spin.add(call);
                                ...
                                //hiện tại không có agent nào rảnh thì lưu vào list các cuộc gọi đang chờ
                                    ...
                                    Call call = new Call(null,0,getStartTime(list_csv,current),current,null,null,null,Double.toString(spin_call));
                        ...
                        if(checkCallAgent(list_agent,current)!=null){
                            ...
                            if(call!=null){
                                ...
                                if(time_differ <= 10){
                                    ...
                                    Call call1 =  new Call(Integer.toString(agent1.id),0,call.call_at,call.pick_up_at,agent1.hangup,hangup_at,"connected",Double.toString(spin_call));
                                    //list_kq.add(call1);
                                    //if(call1.call_at.isBefore(LocalTime.of(21,0,3))){
                                        list_kq.add(call1);
                                        list_spin.add(call1);
                                    //}
                                    agent1.status = "0";
                                ...
                                //sau 10s thì bị abandon
                                    ...
                                    updateStatusAgent(list_agent,agent1);
                                    list_wait.remove(call);
                                    Call call1 = new Call(null,0,call.call_at,call.pick_up_at,null,hangup_at,"abandon",Double.toString(spin_call));
                                    //list_kq.add(call1);
                                    //if(call1.call_at.isBefore(LocalTime.of(21,0,3))){
                                        list_kq.add(call1);
                                        list_spin.add(call1);
...
//        }
        ...
        try (FileWriter writer = new FileWriter("E:\\ContactCenter\\Contact_Center\\data.csv")) {
            // Ghi header
